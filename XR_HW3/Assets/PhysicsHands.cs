...
public class PhysicsHands : MonoBehaviour
{
    ...
    [SerializeField] Claws claws;

    Rigidbody _rigidbody;
    Vector3 _previousPosition;
    bool _isColliding;
    bool _isHandPinned = false;
    ...
    void FixedUpdate()
    {
        ...
        if (_isHandPinned && claws.IsGripping)
        {
            transform.position = _pinnedPosition;
            transform.rotation = _pinnedRotation;           
        }
    ...
    if (collision.gameObject.CompareTag("Climbable") && claws.IsGripping)
    {
        _pinnedPosition = transform.position;
        _pinnedRotation = transform.rotation;
        _isHandPinned   = true;
    }
    }
